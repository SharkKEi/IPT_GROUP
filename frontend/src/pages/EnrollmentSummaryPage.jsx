...
export default function EnrollmentSummaryPage() {
  ...
  // Match by student_number since summary uses student_number as student_id
  ...
  const openStudentDrawer = (student) => {
    setDrawer({
      type: 'student',
      student,
      enrollments: getStudentEnrollments(student.student_id),
    });
  };
  ...
  return (
    <div className="min-h-screen bg-animated overflow-x-hidden">
      <div className="relative z-10 px-6 py-10 lg:px-12">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={fetchAll} className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold text-white transition hover:bg-white/20">
              ↺ Refresh
            </button>
          </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-[#0f1729] border-l border-white/10 shadow-2xl z-50 transition-transform duration-300 ease-in-out overflow-y-auto ${drawer ? 'translate-x-0' : 'translate-x-full'}`}>
        {drawer && (
          <div className="p-8">
            <button onClick={() => setDrawer(null)} className="mb-6 inline-flex items-center gap-2 text-sm text-white/50 hover:text-white transition">
              ✕ Close
            </button>
            ...
            {drawer.type === 'subject' && (
              <>
                <div className="mb-6">
                  <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-2xl mb-4 shadow-lg">
                    📚
                  </div>
