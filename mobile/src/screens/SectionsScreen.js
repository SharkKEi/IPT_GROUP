...
export default function SectionsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const secArray = Array.isArray(sec) ? sec : sec.results || [];
      const subArray = Array.isArray(sub) ? sub : sub.results || [];
      setSections(secArray); setSubjects(subArray);
      if (!form.subject && subArray[0]) setForm((f) => ({ ...f, subject: String(subArray[0].id) }));
