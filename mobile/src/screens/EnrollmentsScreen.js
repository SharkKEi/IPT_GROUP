...
export default function EnrollmentsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const stuArray = Array.isArray(stu) ? stu : stu.results || [];
      const subArray = Array.isArray(sub) ? sub : sub.results || [];
      const enrArray = Array.isArray(enr) ? enr : enr.results || [];
      setStudents(stuArray);
      setSubjects(subArray);
      setEnrollments(enrArray);
      if (!selectedStudent && stuArray[0]) setSelectedStudent(String(stuArray[0].id));
      if (!selectedSubject && subArray[0]) setSelectedSubject(String(subArray[0].id));
