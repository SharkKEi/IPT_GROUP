...
export default function DashboardScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const stuArr = Array.isArray(students) ? students : students.results || [];
      const subArr = Array.isArray(subjects) ? subjects : subjects.results || [];
      const secArr = Array.isArray(sections) ? sections : sections.results || [];
      const enrArr = Array.isArray(enrollments) ? enrollments : enrollments.results || [];
      setStats({
        students: stuArr.length || 0,
        subjects: subArr.length || 0,
        sections: secArr.length || 0,
        enrollments: enrArr.length || 0,
      });
    } catch (e) {
